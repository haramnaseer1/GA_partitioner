...
print(f"\n{'='*80}")
print("TESTING FORMULA: execution_time = processing_time / clock_speed")
print(f"{'='*80}")
...
# Assume all tasks run on processor 51 (first FPGA)
test_proc = 51
...
for task_id in range(5):
    ...
    calc_duration = proc_time / clock_hz
    print(f"{task_id:<6} {proc_time:<12} {calc_duration:<18.10e}")
    
# Compare with actual solution
sol = json.load(open('solution/T2_var_001_ga.json'))
print(f"\n{'='*80}")
print("ACTUAL DURATIONS FROM SOLUTION:")
print(f"{'='*80}")
...
for task_sol in [t for t in sol if t['task_id'] < 5]:
    ...
    print(f"{task_id:<6} {node_id:<6} {duration:<18.10e}")
